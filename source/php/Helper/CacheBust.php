...
namespace WPGraphQLMunicipio\Helper;
...
class CacheBust
{
    ...
    {
        $jsonPath = WPGRAPHQLMUNICIPIO . apply_filters('WPGRAPHQLMUNICIPIO/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . WPGRAPHQLMUNICIPIO . ' and run gulp. See '. WPGRAPHQLMUNICIPIO . 'README.md for more info.</div>';
        }
    }
}
